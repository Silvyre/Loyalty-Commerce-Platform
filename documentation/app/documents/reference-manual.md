...
Each set of credentials include a MAC key identifier and a MAC key. The MAC key identifier uniquely identifies the MAC key. The MAC key
...
would any private key. The LCP currently supports hashing with SHA1 and SHA-256. The hash algorithm used will be auto detected by the LCP based on length.
...
- **Extension:** Blank for GET and DELETE request. For PUT and POST requests,
  ...
  with the request body and hash it with SHA-256.

This normalized request string and the MAC key are fed into the HMAC-SHA256
...
Here is an example of using the HMAC-SHA256 algorithm to generate the signature in Python:
