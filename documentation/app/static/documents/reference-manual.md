...
As a Loyalty Partner, you can integrate with the LCP with four API calls.
...
When an application executes a member validation, credit, or debit operation to you - the Loyalty Partner - via the LCP, the LCP will find the pre-configured URL for you and that specific operation and send an HTTP request to that URL.

### Validate a Member

The basic API call for all LCP partners is the member validation (MV). LCP applications will submit an MV request to determine if a member exists and check the balance of a loyalty program member's account.
...
<table>
  ...
  <tbody>
    <tr>
      <td>order</td>
      <td><b>OPTIONAL</b>: The expanded order resource (if one is present) that this member validation is executed as part of.</td>
    </tr>
    <tr>
      <td>firstName</td>
    ...
    <tr>
      <td>password</td>
      <td>The password for the member’s account.</td>
    </tr>
    <tr>
      <td>zip</td>
      <td>The zip code or postal code for the member’s account.</td>
    </tr>
  </tbody>
</table>
...
A successful MV response will return member details similar to those below if the member exists and is valid for use. You can return additional details to LCP applications to enable further customization for features such as promotions.

    200 OK
    {
     "status": "success|pending", //pending is only for Award Wallet
     "memberId": "12345678",
...
Transfer into and out of members' accounts in one call

An LP configured in the LCP may wish to provide a service to allow LCP applications to perform one shot transfers.
...
<table>
  ...
  <tbody>
    <tr>
      <td>callback</td>
      <td>URI of the LCP Credit or Debit that would need to be updated at a later point should the initial status in the response from the LP API be "Pending"</td>
      <td>Y</td>
    </tr>
    <tr>
      <td>order</td>
      <td>The details of the order that this debit/credit is executed as part of.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>amount</td>
      <td>The number of points to be removed from the member's account and added to the recipient's. This number is negative.</td>
      <td>Y</td>
    </tr>
    <tr>
      <td>pic</td>
      <td>The product identification code.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>member/firstName</td>
      <td>The first name of the loyalty program member transacting.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>member/lastName</td>
      <td>The last name of the loyalty program member transacting.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>member/memberId</td>
      <td>The member ID of the loyalty program member transacting.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>recipient/firstName</td>
      <td>The first name of the loyalty program member getting the points.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>recipient/lastName</td>
      <td>The last name of the loyalty program member getting the points.</td>
      <td>N</td>
    </tr>
    <tr>
      <td>recipient/memberId</td>
      <td>The member ID of the loyalty program member getting the points.</td>
      <td>N</td>
    </tr>
  </tbody>
</table>
...
The transaction id and the status with an optional statusMessage.
...
An LP configured in the LCP may wish to provide a service to allow LCP applications to debit and/or credit a member’s account.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>amount</td>
      <td>The number of points to be added or removed to or from the member's account. This number is either positive or negative.</td>
      <td>Y</td>
    </tr>
    <tr>
      <td>pic</td>
...
An LP configured in the LCP may wish to provide a service to allow LCP applications to retry a failed transaction.
...
<table>
  ...
  <tbody>
    <tr>
      <td>transactionId</td>
      <td>The ID of the initial transaction that is being reattempted. This should be used to ensure that indeed the original transaction was not actually successful.</td>
      <td>Y</td>
    </tr>
  </tbody>
</table>
...
Each call is authenticated with an encrypted username/password combination.